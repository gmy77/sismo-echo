...
<script>
window.__f4ml=[];window.__f4cb=[0,0,0,0,0,0,0];window.__f4lg=0;
fetch('/api/f4strategy').then(function(r){return r.json();}).then(function(d){window.__f4lg=d.games||0;var w=d.cW||[0,0,0,0,0,0,0],l=d.cL||[0,0,0,0,0,0,0];window.__f4cb=w.map(function(v,i){var n=v+l[i];return n>0?(v-l[i]*0.7)/Math.max(n,5)*12:0;});var el=document.getElementById('lrn');if(el)el.textContent=window.__f4lg>0?'🧠 AI: '+window.__f4lg+' partite elaborate':'🧠 AI: nuovo — nessun dato ancora';}).catch(function(){var el=document.getElementById('lrn');if(el)el.textContent='';});
window.__f4end=function(w){if(!w)return;fetch('/api/f4learn',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({moves:window.__f4ml.slice(),winner:w})}).then(function(r){return r.json();}).then(function(d){window.__f4lg=d.games||window.__f4lg;var el=document.getElementById('lrn');if(el)el.textContent='🧠 AI: '+window.__f4lg+' partite elaborate';}).catch(function(){});};
